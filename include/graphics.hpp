...
namespace console {
/**
 * @brief Enumeration of builtin colors for ANSI escape codes, along with other
 * graphics modes
 *
 */
enum Graphics {
...
std::string bold(const std::string& str) {
    if (isANSIEnabled()) return "\033[1m" + str + "\033[22m";
    return str;
}
...
std::string faint(const std::string& str) {
    if (isANSIEnabled()) return "\033[2m" + str + "\033[22m";
    return str;
}
...
std::string italic(const std::string& str) {
    if (isANSIEnabled()) return "\033[3m" + str + "\033[23m";
    return str;
}
...
std::string underline(const std::string& str) {
    if (isANSIEnabled()) return "\033[4m" + str + "\033[24m";
    return str;
}
...
std::string blink(const std::string& str) {
    if (isANSIEnabled()) return "\033[5m" + str + "\033[25m";
    return str;
}
...
std::string inverse(const std::string& str) {
    if (isANSIEnabled()) return "\033[7m" + str + "\033[27m";
    return str;
}
...
std::string hidden(const std::string& str) {
    if (isANSIEnabled()) return "\033[8m" + str + "\033[28m";
    return str;
}
...
std::string strikethrough(const std::string& str) {
    if (isANSIEnabled()) return "\033[9m" + str + "\033[29m";
    return str;
}
...
std::string set(const std::string& str, std::initializer_list<Graphics> list) {
    ...
    if (isANSIEnabled()) return "\033[" + itemStr + "m" + str + "\033[0m";
    return str;
}
