...
void eraseFromCursorToEOS() {
    ...
    DWORD cellsToClear =
        info.csbi.dwSize.X *
            (info.csbi.dwSize.Y - info.csbi.dwCursorPosition.Y - 1) +
        (info.csbi.dwSize.X - info.csbi.dwCursorPosition.X);
    DWORD charsWritten;
    COORD startCoord = info.csbi.dwCursorPosition;
    FillConsoleOutputCharacter(info.hConsole, ' ', cellsToClear, startCoord,
                               &charsWritten);
    FillConsoleOutputAttribute(info.hConsole, info.csbi.wAttributes,
                               cellsToClear, startCoord, &charsWritten);
...
#ifdef __WIN32
    ConsoleInfo info = getConsoleInfo();
    DWORD cellsToClear = info.csbi.dwCursorPosition.Y * info.csbi.dwSize.X +
                         info.csbi.dwCursorPosition.X + 1;
    DWORD charsWritten;
