 ...
 * @param foreground Whether to set the foreground or background color (default
 * is foreground)
 */
std::string colorize(const std::string& str, unsigned char color,
                     bool foreground = true) {
    if (isANSIEnabled())
        return "\033[" + std::to_string(foreground ? 38 : 48) + ";5;" +
 ...
 * is foreground)
 */
std::string colorize(const std::string& str, unsigned char r, unsigned char g,
                     unsigned char b, bool foreground = true) {
    if (isANSIEnabled())
        return "\033[" + std::to_string(foreground ? 38 : 48) + ";2;" +
               std::to_string(r) + ";" + std::to_string(g) + ";" +
               std::to_string(b) + "m" + str + "\033[0m";
    return str;
}
...
void setColor(Graphics color) {
    if (isANSIEnabled()) std::cout << "\033[" << std::to_string(color) << "m";
}
...
void setColor(unsigned char color, bool foreground = true) {
    if (isANSIEnabled())
        std::cout << "\033[" << (foreground ? "38" : "48") << ";5;"
...
void setColor(unsigned char r, unsigned char g, unsigned char b,
              bool foreground = true) {
    if (isANSIEnabled())
        std::cout << "\033[" << (foreground ? "38" : "48") << ";2;"
                  << std::to_string(r) << ";" << std::to_string(g) << ";"
                  << std::to_string(b) << "m";
}
}  // namespace console
